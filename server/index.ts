...
async function proxyOllamaChat(
  ...
  if (!isSameOriginRequest(request)) {
    sendJson(response, 403, { error: 'Cross-origin chat requests are not allowed.' });
    return;
  }
  ...
  const contentType = getFirstHeaderValue(request.headers['content-type']) || '';
  ...
  response.writeHead(ollamaResponse.status, {
    ...
    'Content-Type': ollamaResponse.headers.get('content-type') || 'application/json; charset=utf-8',
...
async function serveStaticFile(pathname: string, response: ServerResponse): Promise<void> {
  ...
  response.writeHead(200, {
    'Cache-Control': fileExtension === '.html' ? 'no-cache' : 'public, max-age=31536000, immutable',
